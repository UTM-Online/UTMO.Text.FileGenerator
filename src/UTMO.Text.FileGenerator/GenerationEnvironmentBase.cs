    ...
    public class GenerationEnvironmentBase : ITemplateGenerationEnvironment, ITemplateGenerationRenderer
    {
        ...
        public virtual void Generate(bool suppressFinalOutput = false)
        {
            ...
            foreach (var item in this.ResourcesInternal)
            {
                ...
                Console.Write($"Generating Resource: \"{item.ResourceTypeName}/{item.ResourceName}\" with template: \"{item.TemplatePath}\".");
                var timer = new Stopwatch();
                timer.Start();
                this.Renderer.GenerateFile(item.TemplatePath, item.ProduceOutputPath(this.OutputPath), item);
                timer.Stop();
                Console.WriteLine($" Rendering took {timer.ElapsedMilliseconds}ms ({timer.Elapsed.TotalSeconds} seconds).");
            ...
            {
                Console.WriteLine($"Generation complete. Output path: \"{this.OutputPath}\".");
            }
        }
    }
}